...
**Before adding or editing an endpoint's return type, read `.claude/skills/DTO_RETURN_TYPES.md`.** Controller return types must be a DTO class — never a primitive, never `DtoType | null`. Wrap primitives in a DTO; for "not found" use `NotFoundException` or a wrapper DTO.
