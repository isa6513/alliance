...
describe('Users (e2e)', () => {
  ...
  describe('signup social proof', () => {
    it('returns five profiles, friends of referrer first when code matches', async () => {
      const inviter = await userRepo.save(
        userRepo.create({
          name: 'Social Proof Inviter',
          email: 'socialproof.inviter@test.com',
          password: 'Password123!',
          profilePicture: 'social-proof-inviter',
        }),
      );
      const friendLo = await userRepo.save(
        userRepo.create({
          name: 'Social Proof Friend Lo',
          email: 'socialproof.friend.lo@test.com',
          password: 'Password123!',
          profilePicture: 'social-proof-friend-lo',
        }),
      );
      const friendHi = await userRepo.save(
        userRepo.create({
          name: 'Social Proof Friend Hi',
          email: 'socialproof.friend.hi@test.com',
          password: 'Password123!',
          profilePicture: 'social-proof-friend-hi',
        }),
      );
      await userService.makeFriendsAutomated(inviter.id, friendLo.id);
      await userService.makeFriendsAutomated(inviter.id, friendHi.id);

      const fillerEmails = [
        'socialproof.fill1@test.com',
        'socialproof.fill2@test.com',
        'socialproof.fill3@test.com',
        'socialproof.fill4@test.com',
      ];
      for (let i = 0; i < fillerEmails.length; i++) {
        await userRepo.save(
          userRepo.create({
            name: `Social Proof Filler ${i}`,
            email: fillerEmails[i],
            password: 'Password123!',
            profilePicture: `social-proof-fill-${i}`,
          }),
        );
      }

      const res = await request(ctx.app.getHttpServer())
        .get('/user/signupSocialProof')
        .query({ code: inviter.referralCode });

      expect(res.status).toBe(200);
      expect(res.body.profiles).toHaveLength(5);
      const ids = res.body.profiles.map((p: { id: number }) => p.id);
      const loFirst = Math.min(friendLo.id, friendHi.id);
      const hiSecond = Math.max(friendLo.id, friendHi.id);
      expect(ids[0]).toBe(loFirst);
      expect(ids[1]).toBe(hiSecond);
      for (const p of res.body.profiles) {
        expect(p.profilePicture).toBeTruthy();
      }

      const noCode = await request(ctx.app.getHttpServer()).get(
        '/user/signupSocialProof',
      );
      expect(noCode.status).toBe(200);
      expect(noCode.body.profiles).toHaveLength(5);
    });
  });
