...
const resolveItemImage = (image?: string) => {
    ...
    if (image.startsWith("http") || image.startsWith("data:") || image.startsWith("file:")) {
        return image;
    }
...
const openWebLocation = (location: string) => {
    const url = location.startsWith("http")
        ? location
        : `https://worldalliance.org${location.startsWith("/") ? "" : "/"}${location}`;
...
export default function SearchScreen() {
    ...
    const handleChooseItem = useCallback((item: SearchItemDto) => {
                ...
                {items.length > 0 && (
                    <View className="border border-zinc-200 rounded mt-3 bg-white overflow-hidden">
                        {categoriesWithItems.map((category, categoryIndex) => (
                            <View
                                key={category}
                                className={categoryIndex === 0 ? "py-3" : "border-t border-zinc-200 py-3"}
                            >
                                    ...
                                    {itemsByCategory[category]?.map((item) => {
                                        ...
                                        return (
                                            <TouchableOpacity
                                                ...
                                                activeOpacity={0.7}
                                                className={`flex-row items-center gap-3 px-3 py-2 rounded ${isSelected ? "bg-zinc-50" : ""
                                                    }`}
                                            >
                                                {item.type === "user" ? (
                                                    <ProfileImage pfp={item.image ?? null} size="small" />
                                                ) : itemImage ? (
                                                    <Image
