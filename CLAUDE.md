...
- Do not modify types.gen.ts or sdk.gen.ts. These will be autogenerated when new server endpoints are created - you can write the frontend such that it will interface with the new autogenerated once generated later, or try running `bun run gen-api` in the root folder to regenerate after you add server endpoints.
